...
def positive_assert(first_name):
    user_body = get_user_body(
        first_name)  # La versión actualizada del cuerpo de solicitud con el nombre "Aa" se guarda en la variable "user_body"
    user_response = sender_stand_request.post_new_user(
        user_body)  # El resultado de la solicitud relevante se guarda en la variable "user_response"

    assert user_response.status_code == 201  # Comprueba si el código de estado es 201
    assert user_response.json()[
               "authToken"] != ""  # Comprueba que el campo authToken está en la respuesta y contiene un valor
...
def negative_assert_symbol(first_name):
    ...
    assert response.json()["message"] == ('Has introducido un nombre de usuario no válido. '
                                          ...
                                          'la longitud debe ser de 2 a 15 caracteres.')  # Se pueden utilizar \ para salto de linea o encerrar todo entre paréntesis.
...
def negative_assert_no_first_name(user_body):
    ...
    assert response.json()[
               "message"] == 'No se han aprobado todos los parámetros requeridos'  # El mensaje indicado es diferente a la respuesta real: 'No se enviaron todos los parámetros necesarios'
...
def test_create_user_has_number_in_first_name_get_error_response():
    print('\n\tPRUEBA 7: Para un nombre de usuario que contiene números "123":')
...
def test_create_user_empty_first_name_get_error_response():
    print('\n\tPRUEBA 9: Para un nombre de usuario vacío:')
    user_body = get_user_body("")
    negative_assert_no_first_name(user_body)


def test_create_user_number_type_first_name_get_error_response():
    print('\n\tPRUEBA 10: Para un nombre de usuario con datos tipo numérico:')
